...
<style>
  ...
  /* Hide material icon text that bleeds into expander labels */
  ...
  /* Fix keyboard_double icon at top of sidebar */
  [data-testid="stSidebar"] [data-testid="collapsedControl"] {{
      color: #e8e2d5 !important;
  }}
  /* Suppress raw icon text */
...
def apply_chart_layout(fig, **extra):
    """Apply CHART_LAYOUT and force all axis/annotation text dark."""
    ...
    fig.update_annotations(font_color="#111111")
    return fig
...
with st.sidebar:
    ...
    with st.expander("About", expanded=False):
        st.markdown("""
Compare and analyze 2–5 stocks using historical price data. Features include price & return analysis, risk & distribution analysis, correlation & diversification analysis, and a two-asset portfolio explorer.
        """)
...
- Benchmark: S&P 500 (`^GSPC`)
    ...
    with st.expander("Sources", expanded=False):
        st.markdown("""
This stock analysis app was built by Shelby Riggs and sourced from Yahoo Finance via `yfinance`. Adjusted closing prices account for dividends and splits.
        """)
...
with tab1:
    ...
    st.plotly_chart(fig_price(fig_price, use_container_width=True)
    else:
    ...
    st.plotly_chart(fig_wealth(fig_wealth, use_container_width=True)
...
with tab2:
    ...
    st.plotly_chart(fig_rv(fig_rv, use_container_width=True)
    ...
    st.plotly_chart(fig_hist(fig_hist, use_container_width=True)
    ...
    st.plotly_chart(fig_qq(fig_qq, use_container_width=True)
    ...
    st.plotly_chart(fig_box(fig_box, use_container_width=True)
...
with tab3:
    ...
    st.plotly_chart(fig_heat(fig_heat, use_container_width=True)
    ...
    st.plotly_chart(fig_scatter(fig_scatter, use_container_width=True)
    ...
    st.plotly_chart(fig_rc(fig_rc, use_container_width=True)
    ...
    st.plotly_chart(fig_pe(fig_pe, use_container_width=True)
    ...
    st.plotly_chart(fig_ef(fig_ef, use_container_width=True)
