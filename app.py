...
# ── Palette ───────────────────────────────────────────────────────────────────
STOCK_COLORS = [
    "#6b1a2e",   # deep burgundy
    "#3d1a3d",   # dark purple
    "#0a3d3d",   # deep teal
    "#1a4d35",   # forest green
    "#8a5a44",   # warm brown (5th stock fallback)
]
BENCHMARK_COLOR = "#6e6e6e"   # grey — S&P 500
EQ_WEIGHT_COLOR = "#9e9e9e"   # lighter grey — equal-weight portfolio
...
<style>
  ...
  .stTabs [data-baseweb="tab"]:hover {{
      background-color: #0d5858 !important;
      color: #ffffff !important;
  }}
...
CHART_LAYOUT = dict(
    ...
    font=dict(family="EB Garamond, Georgia, serif", size=13, color="#c9d1d9"),
    title_font=dict(family="EB Garamond, Georgia, serif", size=16, color="#e6edf3"),
    ...
    legend=dict(
        ...
        bordercolor="#2a2f3e",
        borderwidth=1,
        font=dict(family="EB Garamond, Georgia, serif", size=12),
    ),
)
...
with tab3:
    ...
    fig_heat.update_layout(**CHART_LAYOUT, coloraxis_colorbar_title="Correlation")
