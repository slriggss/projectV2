...
_tab_labels = ["Price & Returns", "Risk & Distribution", "Correlation & Diversification", "ESG Analysis", "Additional Information"]
...
with tab4:
    ...
    st.markdown(
        "ESG scores are sourced from **Sustainalytics** via Yahoo Finance. "
        "Scores reflect ESG risk exposure — **lower scores indicate less risk**. "
        "Not all tickers have ESG data available; large-cap stocks have the best coverage."
    )

    @st.cache_data(ttl=3600)
    def fetch_esg(tickers: tuple) -> dict:
        results = {}
        for tkr in tickers:
            try:
    ...
    with st.spinner("Fetching ESG data…"):
        esg_data = fetch_esg(tuple(stock_cols))

    if not esg_data:
        st.warning(
            "No ESG data found for the selected tickers. "
            "Try large-cap stocks like AAPL, MSFT, GOOGL, AMZN, or JPM."
        )
    else:
        # ── Build summary table ───────────────────────────────────────────────
        ESG_KEYS = {
            "totalEsg":           "Total ESG Score",
            "environmentScore":   "Environment (E)",
        ...
        rows = {}
        for tkr, sus in esg_data.items():
            row = {}
            for key, label in ESG_KEYS.items():
                try:
                    val = sus.loc[key, "Value"] if key in sus.index else None
                    row[label] = val
                except Exception:
                    row[label] = None
            rows[tkr] = row

        esg_df = pd.DataFrame(rows).T
        available_tickers_esg = list(esg_df.index)
        missing_esg = [t for t in stock_cols if t not in available_tickers_esg]
        if missing_esg:
            st.info(f"ESG data not available for: **{', '.join(missing_esg)}**")

        # ── Score bar chart ───────────────────────────────────────────────────
        st.subheader("ESG Score Comparison")
        score_cols = ["Environment (E)", "Social (S)", "Governance (G)"]
        score_df = esg_df[score_cols].apply(pd.to_numeric, errors="coerce").dropna(how="all")

        if not score_df.empty:
            fig_esg_bar = go.Figure()
            bar_colors = {"Environment (E)": "#1a5c35", "Social (S)": "#5c2d6e", "Governance (G)": "#0a5c5c"}
            for col in score_cols:
                if col in score_df.columns:
                    fig_esg_bar.add_trace(go.Bar(
                        name=col,
                        x=score_df.index.tolist(),
                        y=score_df[col].tolist(),
                        marker_color=bar_colors[col],
                        text=score_df[col].round(1).tolist(),
                        textposition="outside",
                        textfont=dict(color="#111111", family="EB Garamond, Georgia, serif"),
                    ))
            apply_chart_layout(fig_esg_bar,
                title="E, S, G Component Scores by Company (Lower = Less Risk)",
                xaxis_title="Ticker",
                yaxis_title="Score",
                barmode="group",
            )
        ...
        # ── Total ESG score bar ───────────────────────────────────────────────
        total_df = esg_df[["Total ESG Score"]].apply(pd.to_numeric, errors="coerce").dropna()
        if not total_df.empty:
            st.subheader("Total ESG Risk Score")
            ...
            def esg_risk_color(score):
                if pd.isna(score): return BENCHMARK_COLOR
                if score < 10:  return "#1a5c35"
                if score < 20:  return "#0a5c5c"
                if score < 30:  return "#c4a35a"
                if score < 40:  return "#8b1a32"
                return "#3d1a3d"

            fig_total = go.Figure()
            for tkr in total_df.index:
                score = total_df.loc[tkr, "Total ESG Score"]
                fig_total.add_trace(go.Bar(
                    x=[tkr], y=[score],
                    name=tkr,
                    marker_color=esg_risk_color(score),
                    text=[f"{score:.1f} — {esg_risk_label(score)}"],
                    textposition="outside",
                    textfont=dict(color="#111111", family="EB Garamond, Georgia, serif"),
                ))
            apply_chart_layout(fig_total,
                title="Total ESG Risk Score by Company (Lower = Less Risk)",
                xaxis_title="Ticker",
                yaxis_title="Total ESG Score",
                showlegend=False,
            )
            # Add risk band reference lines
            for level, val, color in [("Low", 10, "#1a5c35"), ("Medium", 20, "#0a5c5c"),
                                       ("High", 30, "#c4a35a"), ("Severe", 40, "#8b1a32")]:
                fig_total.add_hline(y=val, line_dash="dot", line_color=color,
                                    annotation_text=level, annotation_position="right",
                                    annotation_font_color=color)
            st.plotly_chart(fig_total, use_container_width=True)

        # ── ESG vs Financial Performance scatter ──────────────────────────────
        st.subheader("ESG Score vs. Annualized Return")
        if not total_df.empty:
            stats_for_esg = summary_statistics(returns_stocks)
            common = [t for t in total_df.index if t in stats_for_esg.index]
            if len(common) >= 2:
                esg_scores  = [total_df.loc[t, "Total ESG Score"] for t in common]
                ann_returns = [stats_for_esg.loc[t, "Ann. Return"] for t in common]
                ann_vols    = [stats_for_esg.loc[t, "Ann. Volatility"] for t in common]

                fig_esg_scatter = go.Figure()
                for i, tkr in enumerate(common):
                    fig_esg_scatter.add_trace(go.Scatter(
                        x=[esg_scores[i]], y=[ann_returns[i]],
                        mode="markers+text",
                        name=tkr,
                        text=[tkr],
                        textposition="top center",
                        textfont=dict(color="#111111", family="EB Garamond, Georgia, serif", size=12),
                        marker=dict(
                            color=stock_color(stock_cols, tkr),
                            size=14,
                ...
                apply_chart_layout(fig_esg_scatter,
                    title="ESG Risk Score vs. Annualized Return",
                    xaxis_title="Total ESG Risk Score (Lower = Less Risk)",
                    yaxis_title="Annualized Return",
                    yaxis_tickformat=".1%",
                    showlegend=False,
                )
                st.plotly_chart(fig_esg_scatter, use_container_width=True)
                st.caption(
                    "Each point represents a selected stock. Lower ESG scores indicate less ESG risk exposure. "
                    "This chart explores whether lower ESG risk correlates with stronger financial returns."
                )
            else:
                st.info("Need at least 2 tickers with ESG data to show the scatter plot.")

        # ── ESG Heatmap ───────────────────────────────────────────────────────
        st.subheader("ESG Pillar Heatmap")
        pillar_cols = ["Environment (E)", "Social (S)", "Governance (G)"]
        pillar_df = esg_df[pillar_cols].apply(pd.to_numeric, errors="coerce").dropna(how="all")
        if not pillar_df.empty:
            fig_esg_heat = px.imshow(
                pillar_df,
                text_auto=".1f",
                color_continuous_scale=[
                    [0.0, "#1a5c35"],
                ...
                aspect="auto",
                title="ESG Pillar Scores (Lower = Less Risk)",
            )
            fig_esg_heat.update_layout(**CHART_LAYOUT, coloraxis_colorbar_title="Score")
            fig_esg_heat.update_xaxes(
                tickfont=dict(color="#333333", family="EB Garamond, Georgia, serif"),
                title_font=dict(color="#111111", family="EB Garamond, Georgia, serif"),
            )
            fig_esg_heat.update_yaxes(
                tickfont=dict(color="#333333", family="EB Garamond, Georgia, serif"),
                title_font=dict(color="#111111", family="EB Garamond, Georgia, serif"),
            )
            st.plotly_chart(fig_esg_heat, use_container_width=True)

        # ── Summary table ─────────────────────────────────────────────────────
        st.subheader("Full ESG Data Table")
        display_esg = esg_df.copy()
        for col in display_esg.columns:
            display_esg[col] = pd.to_numeric(display_esg[col], errors="ignore")
        st.dataframe(display_esg, use_container_width=True)

        st.caption(
            "ESG data sourced from Sustainalytics via Yahoo Finance. "
            "Scores reflect risk exposure as of the most recent assessment. "
            "Controversy levels range from 0 (none) to 5 (severe)."
        )
